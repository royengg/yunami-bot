import { getSessionsMap, type PlayerSession } from "../quickstart/runtime-graph.js";
...
export function getNodeInputs(nodeId: string, partyId?: string): NodeInputs | undefined {
    const key = partyId ? `${partyId}:${nodeId}` : nodeId;
...
export function getVoteSummary(nodeId: string, partyId?: string): VoteSummary | null {
    const inputs = getNodeInputs(nodeId, partyId);
...
function evaluateSimpleMajority(inputs: NodeInputs, choices: Choice[]): OutcomeResult {
    const voteCounts = new Map<string, number>();
    ...
    return {
        ...
        message: winningChoiceId ? `Majority chose: ${winningChoice?.label}` : undefined,
    };
}

function evaluateFirstChoice(inputs: NodeInputs, choices: Choice[]): OutcomeResult {
    ...
    return {
        ...
        message: `First choice: ${choice?.label}`,
    };
}

function evaluateLastChoice(inputs: NodeInputs, choices: Choice[]): OutcomeResult {
    ...
    return {
        nextNodeId: choice?.nextNodeId ?? null,
        message: `Last choice: ${choice?.label}`,
    };
}
...
function evaluateRandom(inputs: NodeInputs, choices: Choice[]): OutcomeResult {
    ...
    return {
        nextNodeId: choice?.nextNodeId ?? null,
        message: `Random selection: ${choice?.label}`,
    };
}
